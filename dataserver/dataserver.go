...
func (ds *DataServer) AddBlock(blk ino.Block, volId uint32) (err error) {
...
func (ds *DataServer) TruncBlock(blk ino.Block, volId uint32, newSize uint32) (err error) {
	vol, exists := ds.volumes[volId]
	if !exists {
		return fmt.Errorf("No volume for volID %d", volId)
	}
...
func (ds *DataServer) BlockReport(volId uint32) (blocks []ino.Block, err error) {
	vol, exists := ds.volumes[volId]
	if !exists {
