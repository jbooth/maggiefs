...
func (n *MemNames) Link(parent uint64, child uint64, name string) (newNode *Inode, err error) {
	return n.Mutate(parent, func(inode *Inode) error {
		_, exists := inode.Children[name]
		if exists {
			return fmt.Errorf("Previous child existed when trying to add new child %s, node %+v", name, parent)
		}
		inode.Children[name] = Dentry{child, time.Now().Unix()}
		inode.Nlink++
		fmt.Printf("Setting new child %s to id %d", name, child)
		return nil
	})
}

// Unlinks the child with the given name
func (n *MemNames) Unlink(parent uint64, name string) (newNode *Inode, err error) {
	return n.Mutate(parent,func (node *Inode) error {
